...
unzip(tract_data_filepath, exdir = "data/source-files/tract/2011-2015") 


data_dictionary <- 
  readxl::read_excel("data/source-files/tract/2011-2015/140/CHAS data dictionary 11-15.xlsx", 
                     sheet = "Table 8") %>% 
...
tr <- 
  read_csv("data/source-files/tract/2011-2015/140/Table8.csv") %>% 
  janitor::clean_names("screaming_snake") %>%
...
ggplot(data = tr_by_afford70ami,
  ...
  scale_y_continuous(breaks = seq(0,2000, by = 500),labels = scales::comma) +
  ...
  annotate("text",x = 2, y = sum(y_cant,y_maybe, y_can/2),
           label = 'atop(atop("", bold("income is 80% AMI or more")), atop("685 households (36% of renters)"))',
           parse = TRUE, colour = pal["can afford"]) +
  annotate("text",x = 2, y = sum(y_cant,y_maybe/2),
           label = 'atop(atop("", bold("income is between 50-80% AMI")), atop("210 households (11% of renters)"))', 
           parse = TRUE, colour = pal["can afford (maybe)"]) +
  annotate("text",x = 2, y = y_cant/2,
           label = 'atop(atop("", bold("income is between 0-50% AMI")), atop("990 households (53% of renters)"))', 
...
ggsave("outputs/tract-2011to2015-renters-by-AMI.png", 
       height = 7.29,
       width = 5.98,
       units = "in", 
       device = "png")
